...
import { LocalCatalogueSection } from '@/components/catalogue/LocalCatalogueSection';
...
export default async function CataloguePage({ searchParams, basePath = '/gestion/catalogue', mode = 'gestion' }: { searchParams: Promise<SearchParams>; basePath?: string; mode?: 'comptoir' | 'gestion' }) {
  ...
  const isLocalCat = resolvedParams.cat === 'produits_ajoutes';

  const { products, total, page, totalPages, error } = isLocalCat
    ? { products: [], total: 0, page: 1, totalPages: 0, error: false }
    : await getProducts(resolvedParams);
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-5">
      ...
      {/* Catégorie "Produits ajoutés" : afficher uniquement les produits locaux */}
      {isLocalCat && <LocalCatalogueSection fullPage />}

      {/* Produits Supabase (toutes les autres catégories) */}
      {!isLocalCat && (
        <>
          {error && (
