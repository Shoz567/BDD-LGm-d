...
const INITIAL_MESSAGE: Message = {
  role: 'assistant',
  content:
    'Bonjour ! Je suis l\'assistant LGm@d. Je peux vous aider sur la plateforme — commandes, catalogue, devis, facturation — et je consulte en temps réel votre base de clients enregistrés.\n\nComment puis-je vous aider aujourd\'hui ?',
};

const QUICK_QUESTIONS = [
  'Liste tous mes patients',
  'Quels patients sont éligibles à l\'APA ?',
...
function getPatients() {
  try {
    return JSON.parse(localStorage.getItem('lgmad_patients') ?? '[]');
  } catch {
    return [];
  }
}
...
export default function GestionChatPage() {
  ...
  const { items: cartItems, total: cartTotal, count: cartCount } = useCart();
    ...
    try {
      const res = await fetch('/api/chat?mode=gestion', {
        ...
        body: JSON.stringify({
          ...
          context: {
            date: new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }),
            pharmacie: 'Aprium',
            panier: {
              articles: cartItems.map((i) => ({
                ...
                quantite: i.quantite,
                prix_ttc: i.prix_ttc,
                sous_total: (i.prix_ttc ?? 0) * i.quantite,
              })),
              nombre_articles: cartCount,
              total_ttc: cartTotal,
            },
